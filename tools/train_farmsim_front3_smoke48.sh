...
USE_GVAD_ATTENTION="${USE_GVAD_ATTENTION:-0}"
GVAD_USE_VISIBILITY="${GVAD_USE_VISIBILITY:-1}"
GVAD_USE_LOCAL_DEFORMABLE="${GVAD_USE_LOCAL_DEFORMABLE:-1}"
GVAD_NUM_HEADS="${GVAD_NUM_HEADS:-8}"
GVAD_ANCHOR_GRID_HEIGHT="${GVAD_ANCHOR_GRID_HEIGHT:-4}"
GVAD_ANCHOR_GRID_WIDTH="${GVAD_ANCHOR_GRID_WIDTH:-8}"
...
PYTHONPATH="$(pwd):${PYTHONPATH:-}" CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES}" \
  ...
  --use-gvad-attention "${USE_GVAD_ATTENTION}" \
  --gvad-use-visibility "${GVAD_USE_VISIBILITY}" \
  --gvad-use-local-deformable "${GVAD_USE_LOCAL_DEFORMABLE}" \
