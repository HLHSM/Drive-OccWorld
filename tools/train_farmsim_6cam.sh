...
WORK_DIR="work_dirs/6cam_$(date +%Y%m%d_%H%M%S)"
CONFIG="projects/configs/farmsim/farmsim_occ_6cam.py"
...
if [[ "${NUM_GPUS}" -eq 1 ]]; then
  PYTHONPATH="$(pwd):${PYTHONPATH:-}" \
    "${PYTHON_BIN}" tools/train.py "${CONFIG}" --work-dir "${WORK_DIR}" --launcher none --deterministic --no-validate \
    --cfg-options "${CFG_OPTIONS[@]}"
else
  PYTHONPATH="$(pwd):${PYTHONPATH:-}" \
    "${PYTHON_BIN}" -m torch.distributed.run --standalone --nproc_per_node="${NUM_GPUS}" \
    tools/train.py "${CONFIG}" --work-dir "${WORK_DIR}" --launcher pytorch --deterministic --no-validate \
    --cfg-options "${CFG_OPTIONS[@]}"
fi
