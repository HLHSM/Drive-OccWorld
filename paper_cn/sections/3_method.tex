...
为此，本文提出几何可见锚点可变形注意力（GVAD）。局部可变形采样与 Deformable DETR\cite{zhu2021deformable} 
的稀疏参考点聚合、以及 DETR3D\cite{wang2022detr3d} 的三维查询投影机制一脉相承，但其对观测可靠性的利用仍是隐式的。
GVAD 在保持输入输出尺寸不变的前提下，将局部可变形路径与可见性加权的全局锚点路径耦合，
使弱可见 BEV 单元获得可靠上下文，同时使直接可见区域仍以局部投影证据为主。
...
\end{equation}
因此，弱可见 BEV 单元更倾向读取可靠锚点，而直接可见区域仍以局部图像投影特征为主。
去可见性消融将锚点替换为普通平均池化；去局部路径消融仅保留锚点上下文。

图\ref{fig:gvad}进一步展示 GVAD 的双路径结构：局部可变形路径保持细粒度投影证据，
可见锚点路径提供可靠区域的长程上下文，门控融合根据单元可见性完成自适应组合。
\begin{figure}[t]
\centering
\includegraphics[width=0.94\linewidth]{figures/gvad_module_ai.png}
\caption{GVAD 模块。输入 BEV token 一方面在局部可变形路径中从多尺度图像读取细节，
另一方面通过可见性加权的 tile pooling 构造全局锚点；
锚点注意力与局部增量经可见性门控残差融合，输出增强的 BEV token。}
\label{fig:gvad}
\end{figure}

\subsection{农业结构自适应混合专家三维占用解码器}\label{sec:agri-amoe}
农田占用空间具有显著的各向异性：作物行与垄沟在地平面上呈现长程连续和周期性交替，作物冠层、土壤和障碍物又在高度方向具有不同的边界形态。特别是株间空隙和作物--自由空间边界往往只占少量体素，若将二维 BEV token 独立地直接映射为各高度类别，解码器难以协调相邻位置的平面细节与垂直结构，预测容易在细粒度区域发生过度平滑。因而，最终占用头不仅需要恢复高度维度，还应按局部三维结构的复杂度自适应地选择合适的感受野与方向建模方式。
