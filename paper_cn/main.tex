...
在真实越野数据集 ORAD-3D 上以AgriOcc进行目标域适配也表现出显著的效率和性能提升。另外在真实大豆田的零样本预测中，AgriOcc也展现出良好的泛化能力。
...
\subsection{真实场景实验}\label{sec:sim2real}
...
\begin{figure}[pos=htbp]
\centering
\includegraphics[width=\linewidth]{realfield_zero_shot_qualitative.png}
...
\emph{查询适配}保留主干参数，仅学习 $200\times200$ 高分辨率 BEV 查询格点；
\emph{细化头适配}保留原有 $100\times100$ GeoVis-BEV 编码器与 Agri-AMoE 解码器，冻结其全部已有权重，
仅在占用输出端训练学习式 $2\times$ 细化头。该头将粗占用 logits 与对应的 BEV 特征投影融合，
输出 $200\times200\times25$ 体素。对照组不使用 $0.2\,\mathrm{m}$ 占用预训练权重，
而是在同一个 $0.1\,\mathrm{m}$ 数据集上从头学习 $200\times200$ 的高分辨率 BEV 与占用预测；
三种设置的结果如表\ref{tab:resolution_transfer}所示。

\begin{table}[pos=htbp]
\centering
\caption{$0.1\,\mathrm{m}$ 体素分辨率的迁移结果（IoU 为\%，时间为 8 epoch 端到端墙钟耗时，含逐 epoch 验证）。
迁移方法先在 $0.2\times0.2\times0.2\,\mathrm{m}$ 数据上训练，再冻结已有主干并适配至 $0.1\times0.1\times0.2\,\mathrm{m}$。}
...
\begin{tabular}{lrrrrrrrr}
\toprule
方法 & 时间 & mIoU & Free & Crop & Soil & Drivable & Other veg. & Other obs. \\
\midrule
0.1 m 从头训练 & 7 h 41 min & 40.63 & 90.83 & 18.71 & 59.68 & 28.90 & 27.64 & 17.99 \\
0.1 m 查询适配（冻结主干） & 6 h 31 min & 42.94 & 89.95 & 21.61 & 61.75 & 30.53 & 30.88 & 22.94 \\
0.1 m 细化头适配（冻结主干） & \textbf{1 h 14 min} & \textbf{46.23} & \textbf{91.81} & \textbf{23.59} & \textbf{66.46} & \textbf{37.89} & \textbf{33.07} & \textbf{24.53} \\
\bottomrule
\end{tabular}
}
\end{table}

表\ref{tab:resolution_transfer}显示，相比不使用 $0.2\,\mathrm{m}$ 占用预训练的从头训练，
两种冻结主干的适配方式均缩短了 8 epoch 端到端时间：查询适配由 7 h 41 min 降至 6 h 31 min（减少 15.2\%），
细化头适配进一步降至 1 h 14 min（减少 84.1\%）。同时，查询适配的 mIoU 提升至 42.94\%，
细化头适配达到最高的 46.23\%，分别比从头训练高 2.31 和 5.60 个百分点；后者在六个语义类别上也均取得最佳 IoU。
这是因为直接以 $200\times200$ 查询格点进行适配，仍需在高分辨率 BEV 平面上重新优化大量查询表征；
它虽然复用了冻结主干的图像--BEV 几何先验，但训练和显存开销仍接近端到端高分辨率建模。
细化头适配则保持 $100\times100$ 编码器的稳定全局上下文，仅以 11,238 个可训练参数学习从 0.2 m 粗预测到 0.1 m 体素的高频残差。
...
本文提出 AgriOcc，用三路前视图像直接预测农业机器人坐标系下的前方三维语义占用。
...
ORAD-3D 迁移结果和大豆田数据上的定性分析进一步表明，AgriOcc具有良好的农业场景理解与真实农田泛化能力。
...
研究选择性激活的高分辨率近场查询，以提升株级作物间隙和细粒度边界的表达；
