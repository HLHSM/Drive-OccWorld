...
本文使用 $C_a=96$、$Z=25$。接下来首先对$F_0$进行全局三维池化后接MLP来计算通道显著性$F$。之后再由$F$的通道均值和最大值计算空间显著性，作为路由的空间线索：
\begin{equation}
...
\end{equation}
这里，$\operatorname{GAP}$ 表示在 $Z$、$H$、$W$ 维度上的全局平均池化，$f_c$ 与 $f_s$ 分别为可学习的通道映射和空间映射MLP，
$\odot$ 为逐元素乘法，$\operatorname{mean}_c$ 与 $\operatorname{max}_c$ 分别表示沿通道维度的平均与最大聚合，
$\bm{S}$ 为逐体素空间显著性图。

为使专家选择与三维结构复杂度对齐，我们接下来计算体积特征在三个空间方向的有限差分能量：
\begin{equation}
\bm{E}=\operatorname{mean}_c\!\left((\nabla_x\bm{F})^2+(\nabla_y\bm{F})^2+(\nabla_z\bm{F})^2\right).
\end{equation}
其中，$\nabla_x$、$\nabla_y$ 和 $\nabla_z$ 分别为三个空间方向的梯度，$\operatorname{mean}_c$ 仍表示通道平均，
故 $\bm{E}$ 为逐体素的局部结构变化强度。之后逐体素路由器以 $[\log(1+\bm{E}),\bm{S}]$ 为输入，
在温度 $\tau=1$ 下输出三个归一化专家权重
$\bm{\alpha}=\operatorname{Softmax}(g([\log(1+\bm{E}),\bm{S}])/\tau)$，其中 $g$ 为可学习的全连接层，
